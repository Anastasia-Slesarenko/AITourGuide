...
def _unique(jpeg: bytes) -> bytes:
    """Делает байты запроса уникальными, дописывая случайный хвост.
    ...
    """
    return jpeg + os.urandom(16)


def _random_known() -> bytes:
    """Случайное known-фото (объект есть в индексе), уникализированное."""
    return _unique(random.choice(_KNOWN_POOL))


def _random_novel() -> bytes:
    """Случайное novel-фото (объекта нет в индексе), уникализированное."""
    return _unique(random.choice(_NOVEL_POOL))
