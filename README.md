...
Двусторонняя проверка сходится. Убрать **query-фото** — качество падает до случайного (Hit@1 0.70 → 0.10, AUROC 0.92 → 0.54; зашумлённый query даёт то же). Убрать **текст** кандидата — метрики почти не меняются (0.70 → 0.69). Значит реранкер решает попарным **зрительным** сравнением query↔candidate, а не эксплуатирует имя/caption — текстовой утечки нет.
