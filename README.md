...
| **LoRA-reranker (эта работа)** | **73.51%** | **0.833** | **81.76%** |

> **Fine-tuning — ключ ко всему.** «Из коробки» VLM в роли ранкера **вредит** (26.98% против 60.74% у ретривера). LoRA-дообучение на попарном сравнении фото при 448px превращает его в лучший компонент системы: **+12.8 п.п. Hit@1** над ретривером и рост распознавания «неизвестных» объектов с ~0 до **81.8%**.

**Что здесь показано как ML-инженерная работа:**
...
- 🗃️ **Пайплайн данных** — сбор, фильтрация, VLM-каптионинг и генерация описаний (6 шагов, S3, майнинг hard-negative «unknown»).
...
| **Hit@3** | 76.35% | 53.86% | **90.46%** |
| **MRR** | 0.711 | 0.465 | **0.833** |
| **Unknown accuracy** | — | 1.51% | **81.76%** |
| **Unknown AUROC** | — | 0.587 | **0.806** |
| **Brier score** ↓ | — | 0.330 | **0.069** |

> Recall@10 ретривера — 93.9%: реранкер работает внутри этого «потолка» и почти полностью восстанавливает правильный порядок (Hit@3 90.5%). Brier score 0.33 → 0.07 показывает, что дообучение не только повысило точность, но и **откалибровало** уверенность.
