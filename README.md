...
Загружаете фото — получаете название объекта, экскурсионное описание и калиброванную уверенность модели. Если объекта нет в галерее, сервис не выдумывает: включает fallback — ищет в интернете (Yandex Image Search, затем Wikipedia) с повторной VLM-верификацией, а при её неудаче честно отвечает «не знаю».
...
*Ранжирование на валидации: 5 746 known-запросов, единый фиксированный набор step6-кандидатов — все три модели на одном знаменателе. Полный пайплайн на тесте, open-set и калибровка — в разделе [Результаты](#результаты).*
...
- **Двухступенчатый retrieve-rerank** — SigLIP-эмбеддинги + FAISS для отбора кандидатов, VLM для точного попарного ранжирования.
...
- **MLOps** — Docker Compose, Prometheus + Grafana + Loki, структурированные логи с correlation ID, CI/CD (ruff, pytest, публикация образа в GHCR, деплой по SSH).
...
4. **Fallback в интернет.** При низкой уверенности параллельно запускается Yandex Image Search, затем Wikipedia, повторная VLM-верификация и перевод описания через Yandex Translate (с английского на русский).

Описание не генерируется, а берётся готовым у найденного объекта (в fallback — из Wikipedia с переводом). Это не RAG: VLM здесь не генерирует ответ, а сравнивает и проверяет. Ранняя версия, где VLM по кандидатам генерировала гид-описание, давала много галлюцинаций — поэтому задача сведена к «найти и верифицировать» вместо «сгенерировать».
...
<details>
<summary><b>Формулы метрик</b></summary>

Обозначения: $N$ — число запросов, $\text{rank}_i$ — позиция истинного объекта в выдаче, $l_{yes}, l_{no}$ — логиты токенов Yes/No, $s$ — уверенность (max $P(yes)$).

| Метрика | Определение | Значение (LoRA) |
|---|---|---|
| Hit@k | $\frac{1}{N}\sum_i \mathbb{1}[\text{rank}_i \le k]$ — доля запросов с истинным объектом в top-k | 73.51% (k=1), 91.70% (k=3) |
| MRR | $\frac{1}{N}\sum_i \frac{1}{\text{rank}_i}$ | 0.833 |
| nDCG@k | $\dfrac{DCG@k}{IDCG@k},\ DCG@k=\sum_{j=1}^{k}\dfrac{rel_j}{\log_2(j+1)}$ | 0.842 (k=3, val) |
| P(yes) | $\dfrac{e^{l_{yes}}}{e^{l_{yes}}+e^{l_{no}}}$ — уверенность реранкера из logprobs | — |
| AUROC | $P(s_{\text{known}} > s_{\text{novel}})$ — площадь под ROC | 0.643 |
| F1-macro | $\frac{1}{2}\bigl(F1_{\text{known}}+F1_{\text{unknown}}\bigr)$ | 0.472 |
| Порог (Youden) | $t^\ast=\arg\max_t\,(TPR_t + TNR_t - 1)$ | 0.473 |
| Balanced accuracy | $\frac{1}{2}(TPR + TNR)$ | 50.7% (e2e) |
| ECE | $\sum_{m}\frac{\lvert B_m\rvert}{N}\,\bigl\lvert \text{acc}(B_m)-\text{conf}(B_m)\bigr\rvert$ | 0.009 (isotonic) |
| Brier | $\frac{1}{N}\sum_i (p_i - y_i)^2$ | 0.298 → 0.201 (сырой → isotonic) |
...
Когда пайплайн выдаёт `P(yes)=0.8`, верен ли top-1 в ~80% случаев. Метрика — ECE по `max P(yes)` против факта «top-1 верен». Калибруем на отвеченных запросах (accepted, conf ≥ порога — только их и показываем; selective prediction), фит на val, ECE — на тесте (held-out).

| ECE ↓ (тест, accepted) | сырые | temperature | **isotonic** | AUROC |
|---|:---:|:---:|:---:|:---:|
| Zero-shot | 0.662 | 0.405 | **0.007** | 0.500 |
| **LoRA** | 0.288 | 0.099 | **0.009** | **0.643** |

На отвеченных запросах реранкер заметно переуверен (raw ECE 0.29). Temperature scaling (один параметр, T=3.84) снижает до 0.10, но isotonic (гибкая монотонная регрессия, фит на val) — до 0.009. Все калибраторы монотонны, поэтому порог и решение known/unknown не меняют.

ECE читать вместе с AUROC: до ~0.007–0.009 доводятся обе модели, но информативна только LoRA (AUROC 0.64) — её уверенность и честна, и различает «в базе / не в базе». Zero-shot откалиброван, но слеп (AUROC 0.50): числа честные, но неинформативные (≈ базовая частота — он и «принимает» почти всё).

В проде API отдаёт **isotonic-калиброванный** `P(correct)` ([data/calibration/](data/calibration/)), а не сырой P(yes) (например, сырой p_yes 0.9 калибруется в 0.49 — реальная точность на этом уровне). Пользователю при этом показывается не точный процент, а бэнд — Высокая / Средняя / Низкая: точные проценты вводят в заблуждение, а бэнд честно передаёт уровень доверия. Границы 0.6 / 0.3 на калиброванной шкале выбраны так, что факт. точность в бэндах — **78% / 44% / 27%**. Решение known/unknown — на сыром скоре (порог Youden 0.473); калибровка монотонна и его не меняет.
...
| no_text (имя и caption убраны) | 0.690 | 0.916 |

Двусторонняя проверка сходится. Убрать query-фото — качество падает до случайного (Hit@1 с 0.70 до 0.10, AUROC с 0.92 до 0.54; зашумлённый query даёт то же). Убрать текст кандидата — метрики почти не меняются (с 0.70 до 0.69). Значит реранкер решает попарным зрительным сравнением query и candidate, а не эксплуатирует имя/caption — текстовой утечки нет.

### End-to-end (тест, полный пайплайн)

Под каждый запрос пайплайн заново ищет по галерее (α=0.8) — production-like оценка, без fallback в интернет. Known = 13 889 запросов из test.json (объект в индексе), unknown = 2 616 фото novel-объектов (1 528 объектов вне индекса). Метка known/unknown — по членству в индексе, поэтому знаменатель фиксирован и одинаков для всех моделей; у каждой модели свой порог (Youden, подобран на val).
...
Как из sweep-прогонов выбрана итоговая конфигурация — [docs/MODEL_SELECTION.md](docs/MODEL_SELECTION.md). Скрипт обучения и конфигурации прогонов — [scripts/experiments/train.py](scripts/experiments/train.py); результаты экспериментов — [scripts/experiments/results/](scripts/experiments/results/).
