...
def evaluate_retrieval_with_reranking(
    ...
    """
    _top_k: list[int] = top_k_list if top_k_list is not None else [1, 3, 5, 10, 15]
    ...
    print(f"   Reranking: {'✅ Включен' if use_reranking else '❌ Выключен'}")
    ...
    if use_reranking:
        print(f"   Initial k: {initial_k} → Final k: {max(_top_k)}")
    ...
    recall_counts = dict.fromkeys(_top_k, 0)
    ...
    for emb_idx, orig_idx in enumerate(
        ...
        if use_reranking:
            ...
            candidates = retriever.search_by_embedding(emb, top_k=initial_k)
            ...
            retrieved = (
                ...
                else candidates[: max(_top_k)]
            )
        else:
            retrieved = retriever.search_by_embedding(emb, top_k=max(_top_k))
    ...
    metrics = {
        f"recall@{k}": round(recall_counts[k] / total_samples, 4) for k in _top_k
    }
...
def compare_reranking(
    ...
    for k in top_k_list:
        ...
        print(f"{key:<15} {without:<15.1%} {with_ce:<15.1%} {improvement:+.1%}")
...
if __name__ == "__main__":
    ...
    output_path = "/home/jupyter/s3/ai-tour-guide/retrieval_reranking_comparison.json"
