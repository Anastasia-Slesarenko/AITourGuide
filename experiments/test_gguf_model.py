...
if _ChatHandlerClass is None:
    print("⚠️  Ни один VLM chat handler не найден — изображения НЕ будут обработаны!")
...
def load_gguf_model(
    ...
    if mmproj_path:
        ...
        if mmproj_path_obj.exists():
            ...
            if verbose:
                print(f"   ℹ️  mmproj найден: {Path(mmproj_path).name} ({mmproj_path_obj.stat().st_size / 1e6:.1f} MB)")
    ...
    if verbose:
        ...
        print(f"   Vision support: {'✅' if has_mmproj else '❌'}")
    ...
    try:
        ...
        if has_mmproj and _ChatHandlerClass is not None:
            ...
            if n_gpu_layers == 0:
                ...
                if verbose:
                    print("   ℹ️  GPU patch applied (use_gpu=False для mmproj)")
            ...
            if verbose:
                print("   ⚠️  chat_handler недоступен, используем mmproj напрямую")
        else:
            if verbose:
                print("   ⚠️  mmproj не найден — модель работает БЕЗ изображений!")
...
def _get_yes_no_ids(llm: Llama) -> tuple:
    ...
    if len(yes_tokens_space) == 1 and len(no_tokens_space) == 1:
        ...
        print(f"   ℹ️  Token IDs: ' Yes'={_YES_TOKEN_ID}, ' No'={_NO_TOKEN_ID}")
...
def _extract_logits_from_scores(
    ...
    try:
        ...
        if isinstance(raw, np.ndarray):
            ...
            row = arr_2d[-1]
            print(
                f"   ℹ️  _scores numpy {arr_2d.shape}, строка [-1]"
            )
        ...
        if isinstance(raw, (list, tuple)) and len(raw) > 0:
            ...
            print(f"   ℹ️  _scores list[{n_tokens}], строка [-1]")
            return True, float(row[yes_id]), float(row[no_id])
        ...
        try:
            ...
            print(f"   ⚠️  _scores неизвестный тип: {type(raw)}")
            return False, 0.0, 0.0

        if total_floats == 0:
            print("   ⚠️  _scores пуст (logits_all=True не сработал?)")
            return False, 0.0, 0.0

        if total_floats % n_vocab != 0:
            print(
                f"   ⚠️  _scores размер {total_floats} "
        ...
        row = arr_2d[-1]
        print(
            f"   ℹ️  _scores ctypes flat [{n_tokens}, {n_vocab}], строка [-1]"
        )
        return True, float(row[yes_id]), float(row[no_id])

    except Exception as e:
        print(f"   ⚠️  Ошибка извлечения логитов из _scores: {e}")
        return False, 0.0, 0.0
...
def get_yes_probability(llm: Llama, messages: List[Dict]) -> float:
    ...
    try:
        ...
        print(f"⚠️ Не удалось получить ID токенов Yes/No: {e}")
        return 0.0
    ...
    try:
        ...
        if captured["logits"] is None:
            print("   ⚠️  logits_processor не вызван → fallback")
        ...
        print(f"⚠️ AttributeError: {e} → fallback")
        return _get_yes_probability_fallback(llm, messages)
    except Exception as e:
        print(f"⚠️ Ошибка (logits_processor): {e} → fallback")
        return _get_yes_probability_fallback(llm, messages)
...
def _get_yes_no_grammar() -> Optional["LlamaGrammar"]:
    ...
    if _YES_NO_GRAMMAR is None:
        try:
            ...
            print(f"⚠️ Не удалось создать grammar: {e}")
...
def rank_candidates(llm: Llama, query_image_path: str, candidates: List[Dict]) -> List[Dict]:
    ...
    for i, cand in enumerate(candidates):
        ...
        if top_image is None:
            print(f"  [{i+1}/{len(candidates)}] ⚠️ Пропуск (нет данных gallery_images)")
...
def main_test(retrieved_candidates: List[Dict], query_image_path: str, true_lid: str):
    ...
    print("=" * 70)
    print(f"⏱️  Общее время оценки: {total_time:.2f} сек")
