...
class Config:
    ...
    # Настройки Yandex AI Studio API
    ...
    concurrent_requests: int = (
        5  # Количество одновременных запросов (уменьшено для стабильности)
    )
...
class GuideDescriptionGenerator:
    ...
    def _build_structured_summary(self, item: dict[str, Any]) -> str:
        ...
        if "landmark_type" in item and isinstance(item["landmark_type"], dict):
            landmark_type = (
                item["landmark_type"].get("ru") or item["landmark_type"].get("en") or ""
            )

        # Краткое описание из Wikidata
        wikidata_description = (
            item.get("wikidata_description_ru")
            or item.get("wikidata_description_en")
            or ""
        )

        wiki = (
            item.get("wikipedia_summary_ru") or item.get("wikipedia_summary_en") or ""
        )
...
"""
    ...
    def _parse_response(self, text: str) -> str | None:
        ...
        if chinese_chars > 0:
            logger.warning(
                f"WARNING - Обнаружены китайские символы: {chinese_chars} символов"
            )
            return None
        ...
        if total_chars > 0 and russian_chars / total_chars < 0.6:
            logger.warning(
                f"WARNING - Текст содержит слишком много нерусских символов: {russian_chars}/{total_chars}"
            )
            return None
    ...
    async def _call_api(
        self, session: aiohttp.ClientSession, prompt: str, attempt: int = 1
    ) -> str | None:
        ...
        try:
            async with self.semaphore:
                ...
                timeout = aiohttp.ClientTimeout(
                    total=120, connect=30, sock_connect=30, sock_read=90
                )
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if attempt < self.config.retry_attempts:
                            logger.info(
                                f"Повторная попытка {attempt + 1}/{self.config.retry_attempts}"
                            )
            ...
            logger.error(
                f"Timeout при запросе к API (попытка {attempt}/{self.config.retry_attempts})"
            )
            self.metrics.api_errors += 1
            ...
            if attempt < self.config.retry_attempts:
                logger.info(
                    f"Повторная попытка {attempt + 1}/{self.config.retry_attempts} через {self.config.retry_delay}с"
                )
                await asyncio.sleep(self.config.retry_delay)
                return await self._call_api(session, prompt, attempt + 1)
            ...
            logger.warning(
                f"Сервер отключился (попытка {attempt}/{self.config.retry_attempts}): {e}"
            )
            self.metrics.api_errors += 1
            ...
            if attempt < self.config.retry_attempts:
                ...
                logger.info(
                    f"Повторная попытка {attempt + 1}/{self.config.retry_attempts} через {delay}с"
                )
            ...
            logger.warning(
                f"Ошибка клиента aiohttp (попытка {attempt}/{self.config.retry_attempts}): {type(e).__name__}: {e}"
            )
            self.metrics.api_errors += 1

            # Повторная попытка при ошибке клиента
            if attempt < self.config.retry_attempts:
                logger.info(
                    f"Повторная попытка {attempt + 1}/{self.config.retry_attempts} через {self.config.retry_delay}с"
                )
            ...
            logger.error(
                f"Неожиданная ошибка при вызове API (попытка {attempt}/{self.config.retry_attempts}): {type(e).__name__}: {e}",
                exc_info=True,
            )
            self.metrics.api_errors += 1

            # Повторная попытка при неизвестной ошибке
            if attempt < self.config.retry_attempts:
                logger.info(
                    f"Повторная попытка {attempt + 1}/{self.config.retry_attempts} через {self.config.retry_delay}с"
                )
    ...
    async def generate_description(
        ...
        if result is None:
            name = (
                item.get("name_ru")
                or item.get("name_en")
                or "Неизвестная достопримечательность"
            )
...
class GuideDescriptionProcessor:
    ...
    async def _atomic_save(self, data: list[dict[str, Any]], output_path: Path):
        ...
        async with self.save_lock:
            temp_fd, temp_path = tempfile.mkstemp(
                dir=output_path.parent, prefix=".tmp_output_", suffix=".json"
            )
            try:
                # Выполняем блокирующую операцию записи в executor
                loop = asyncio.get_event_loop()
                await loop.run_in_executor(None, self._write_json_sync, data, temp_path)
    ...
    async def process_item(
        ...
        if description_text is None:
            ...
            wiki_summary = (
                item.get("wikipedia_summary_ru")
                or item.get("wikipedia_summary_en")
                or ""
            )
            if wiki_summary:
    ...
    async def run(self):
        ...
        with open(self.config.input_path, encoding="utf-8") as f:
            data = json.load(f)
        logger.info(f"Загружено {len(data)} объектов из {self.config.input_path}")
        ...
        if Path(self.config.output_path).exists():
            try:
                ...
                logger.info(
                    f"Загружено {len(existing_results)} существующих результатов"
                )
            except Exception as e:
                ...
                try:
                    for coro in asyncio.as_completed(tasks):
                        try:
                            ...
                            logger.error(
                                f"Ошибка обработки элемента: {type(e).__name__}: {e}",
                                exc_info=True,
                            )
                            self.metrics.objects_failed += 1
                            self.metrics.objects_processed += 1
        ...
        logger.info(f"Ошибок API: {metrics_report['api_errors']}")
        logger.info("-" * 70)
        ...
        if total_processed > 0:
            ...
            ai_rate = (
                (total_success - metrics_report["objects_fallback"]) / total_processed
            ) * 100
            logger.info(f"Общая успешность: {success_rate:.1f}%")
            logger.info(f"AI генерация: {ai_rate:.1f}%")

        logger.info("-" * 70)
        logger.info(f"Результат сохранен в: {self.config.output_path}")
        logger.info("=" * 70)
