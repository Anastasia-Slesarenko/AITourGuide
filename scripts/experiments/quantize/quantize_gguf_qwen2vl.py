...
def create_mmproj_simple(model_dir: Path, output_path: Path):
    ...
    try:
        ...
        if result.returncode == 0 and output_path.exists():
            ...
            print(f"Стандартный конвертер не сработал")
            print(f"   stderr: {result.stderr[:200][:200]}")
            
    except Exception as e:
        print(f"Ошибка создания mmproj: {e}")
...
def main():
    ...
    if quantized:
        ...
        print("Q5_K_M пропущен, использую f16 модель")
        gguf_main_q5 = gguf_main_f16
    
    # 3. Create mmproj
    print("\nШаг 3: Создание mmproj...")
    mmproj_f16 = OUTPUT_BASE / "mmproj-model-f16.gguf"
    
    created = create_mmproj_simple(merged_dir, mmproj_f16)
    
    if not created:
        print("MMProj не создан — модель будет работать ТОЛЬКО с текстом!")
    ...
    if mmproj_f16.exists():
        ...
        print(f"mmproj: НЕ СОЗДАН (работаем без изображений)")
